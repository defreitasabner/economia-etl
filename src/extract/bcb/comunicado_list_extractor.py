...
@ExtractorRegistry.register('comunicados_list')
class ComunicadoListExtractor(Extractor):
    ...
    def extract(self) -> tuple[list[dict], dict]:
        ...
        comunicados = response.json().get('conteudo', [])

        metadata = {
            'url': response.url,
            'query_params': self.__query_params,
            'record_count': len(comunicados),
            'extracted_at': datetime.now().isoformat(),
        }
        return comunicados, metadata

    def __validate_params(self) -> None:
        quantidade = self.__query_params.get('quantidade', 0)
        if quantidade <= 0:
            raise ValueError(
                f"A quantidade de comunicados deve ser um número positivo. Valor fornecido: {quantidade}"
            )
